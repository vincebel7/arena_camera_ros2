...
void ArenaCameraNode::parse_parameters_()
{
  ...
  try {
    ...
    // action_trigger_mode gates TriggerSource=Action0 capture; action_master
    // designates the single node that broadcasts the action command. The
    // device/group key/mask MUST be identical on all six cameras and on the
    // fire command.
    nextParameterToDeclare = "action_trigger_mode";
    action_trigger_mode_activated_ =
        this->declare_parameter("action_trigger_mode", false);
...
void ArenaCameraNode::initialize_()
{
  ...
  if (action_trigger_mode_activated_ && action_master_) {
...
void ArenaCameraNode::run_()
{
  ...
  if (action_trigger_mode_activated_) {
    // Frames arrive only when a scheduled action command fires. Run the
    // GetImage/publish loop in a background thread so this node can still
    // spin and serve /trigger_all (master) and the continuous timer.
  ...
  } else if (!trigger_mode_activated_) {
    publish_images_();
  } else {
    // else ros::spin will
  }
    ...
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  if (!action_trigger_mode_activated_ || !action_master_) {
    ...
    response->message =
        "this node is not the action master in action_trigger_mode";
...
void ArenaCameraNode::set_nodes_()
{
  ...
  set_nodes_gamma_();
  if (action_trigger_mode_activated_) {
    // Synchronized capture via PTP + scheduled GigE Vision action commands.
    // The action command gates every frame, so the free-run AcquisitionFrameRate
    // is intentionally NOT enabled here (see set_nodes_action_trigger_mode_()).
...
void ArenaCameraNode::set_nodes_action_trigger_mode_()
{
  ...
  if (exposure_time_ < 0) {
    log_warn(
        "\taction_trigger_mode: no fixed exposure_time provided. A fixed "
        "(bounded) exposure is recommended for deterministic synchronized "
        "capture under triggering.");
  }
  ...
  log_info(
      std::string(
          "\taction_trigger_mode activated: TriggerSource=Action0 "
